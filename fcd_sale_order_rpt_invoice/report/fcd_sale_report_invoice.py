...
class FcdSaleReportInvoice(models.Model):
    ...
    sale_price_kg = fields.Monetary(readonly=True)
    fcd_purchase_price_kg = fields.Monetary(readonly=True)
    smls_amount_base = fields.Monetary(digits="Product Unit of Measure", readonly=True)
    smls_price_subtotal = fields.Monetary(readonly=True)
    margin_net = fields.Monetary(readonly=True)
    margin_net_invoiced = fields.Monetary(readonly=True)
