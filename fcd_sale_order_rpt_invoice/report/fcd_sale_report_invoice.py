...
class FcdSaleReportInvoice(models.Model):
    ...
    smls_qty_invoiced = fields.Float(readonly=True)
    sale_price_kg = fields.Float(readonly=True)
    fcd_purchase_price_kg = fields.Float(readonly=True)
    smls_amount_base = fields.Monetary(digits="Product Unit of Measure", readonly=True)
    smls_price_subtotal = fields.Float(readonly=True)
    margin_net = fields.Monetary(readonly=True)
    margin_net_invoiced = fields.Float(digits="Product Unit of Measure", readonly=True)
